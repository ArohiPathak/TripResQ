...
export default function FamilyFractureRescue({
  ...
  // Derive human-readable titles from backend rule names
  const RULE_TITLES = {
    CHILD_GUARDIAN: 'Unaccompanied Minor Hazard',
    COHORT_COHESION: 'Family Cohort Fracture',
    BOOKING_DEPENDENCY: 'Lodging & Logistics Desynchronized',
  };
  ...
  ]).map(v => ({ ...v, title: v.title || RULE_TITLES[v.rule] || v.rule }));

  // Extract unified rescue and advance party data from backend simulation
  const unifiedRescue = simulationData?.scenarios?.unified_rescue || {};
  const advanceParty = simulationData?.scenarios?.advance_party || {};
  const unifiedValid = unifiedRescue?.validation?.valid ?? true;
  const advancePartyValid = advanceParty?.validation?.valid ?? false;

  // Derive traveler count from cohort or fallback
  const travelerCount = cohort?.members?.length || airlineSplit.length || 3;
  const childName = cohort?.members?.find(m => m.type === 'CHILD')?.name || 'Aarav';
  const guardianNames = (cohort?.members || []).filter(m => m.type === 'ADULT').map(m => m.name).join(' & ') || 'Rahul & Priya';
  ...
  return (
    ...
    >
        ...
        <div className="relative z-10 flex flex-col gap-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
            <div className="bg-white/5 border border-white/10 p-3.5 rounded-xl flex flex-col gap-1">
              ...
              <span className="text-white/70 text-[11px]">All {travelerCount} travelers on same departure</span>
            </div>
            ...
            <div className="bg-white/5 border border-white/10 p-3.5 rounded-xl flex flex-col gap-1">
              ...
              <span className="font-bold text-white text-sm">{unifiedRescue?.validation?.seating?.status === 'REQUESTED' ? 'Adjacent Requested' : 'Standard'}</span>
              <span className="text-white/70 text-[11px]">{unifiedRescue?.validation?.seating?.preference === 'ADJACENT' ? 'Adjacent seating requested' : 'Seating assignment pending'}</span>
            </div>
            ...
            <div className="bg-white/5 border border-white/10 p-3.5 rounded-xl flex flex-col gap-1">
              ...
              <span className="font-bold text-white text-sm">{unifiedValid ? '100% Protected' : 'At Risk'}</span>
              <span className="text-white/70 text-[11px]">{childName} with {guardianNames}</span>
            </div>

            <div className="bg-white/5 border border-white/10 p-3.5 rounded-xl flex flex-col gap-1">
              <span className="text-emerald-300 font-mono text-[10px] uppercase font-bold flex items-center gap-1">
                <Clock className="w-3 h-3" /> Validation Status
              </span>
              <span className="font-bold text-white text-sm">{unifiedValid ? 'VALID' : 'INVALID'}</span>
              <span className="text-white/70 text-[11px]">{unifiedRescue?.validation?.violations?.length === 0 ? 'Zero violations • Downstream synchronized' : `${unifiedRescue?.validation?.violations?.length} violations detected`}</span>
            </div>
          </div>
      ...
      {/* 🔄 5. ADVANCE PARTY ALTERNATIVE (from backend) */}
      {advancePartyValid && (
        <div className="bg-white p-5 sm:p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col gap-4">
          <div className="border-b border-slate-100 pb-3">
            <div className="text-[11px] font-mono font-bold text-[#287DFA] uppercase tracking-wider flex items-center gap-1.5">
              <Users className="w-3.5 h-3.5" />
              <span>Alternative: Advance Party Route Split</span>
            </div>
            <h3 className="text-base sm:text-lg font-bold text-slate-900 font-serif">
              Send One Guardian Ahead, Keep Child Protected
            </h3>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {Object.entries(advanceParty?.plan?.traveler_assignments || {}).map(([tid, flight]) => {
              const member = cohort?.members?.find(m => m.traveler_id === tid);
              const isChild = member?.type === 'CHILD';
              return (
                <div
                  key={tid}
                  className={`p-4 rounded-xl border flex flex-col gap-2 ${
                    isChild ? 'bg-emerald-50/70 border-emerald-200' : 'bg-slate-50 border-slate-200'
                  }`}
                >
                  <div className="flex items-center gap-2">
                    <span className="text-lg">{isChild ? '👦' : member?.name === 'Priya' ? '👩' : '👨'}</span>
                    <div>
                      <h4 className="font-bold text-sm text-slate-900">{member?.name || tid}</h4>
                      <span className="text-[10px] font-mono text-slate-500 uppercase">{member?.type || 'ADULT'} • {member?.pnr || tid}</span>
                    </div>
                  </div>
                  <div className="text-xs font-mono text-slate-600 flex justify-between">
                    <span>Assigned:</span>
                    <span className="font-bold text-[#287DFA]">{flight}</span>
                  </div>
                  {isChild && (
                    <div className="text-[10px] font-bold text-emerald-700 bg-emerald-100 px-2 py-1 rounded text-center">
                      ✅ With designated guardian
                    </div>
                  )}
                </div>
              );
          ...
          <div className="flex items-center gap-2 text-xs text-slate-500">
            ...
            <span>Advance Party is valid — child remains with at least one guardian. {advanceParty?.validation?.violations?.length === 0 ? 'Zero violations.' : ''}</span>
          </div>
        </div>
      )}
    </motion.div>
