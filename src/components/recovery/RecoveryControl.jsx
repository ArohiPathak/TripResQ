...
export default function RecoveryControl({
  ...
  const [activeMode, setActiveMode] = useState('fracture'); // 'fracture' | 'preferences'
  ...
  const handleApplyPlan = async (planToApply) => {
    ...
    setIsApplying(true);
    try {
      // Call backend with ONLY the selected plan's proposals
      const res = await fetch(`${API_BASE}/trips/${tripId}/apply-plan`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ proposals: plan.proposals || [] })
      });
      ...
      setTimeout(() => {
        if (onPlanApplied) {
          onPlanApplied(data.updated_graph, plan);
        }
      }, 2200);
